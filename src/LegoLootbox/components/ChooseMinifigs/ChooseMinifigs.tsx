...
import fetchMinifigs from '../../gateway/fetchMinifigs';
...
import fetchMinifigParts from '../../gateway/fetchMinifigParts';
...
const filterRandomElements = (elements: MinifigInterface[]) => (
  ...
  elements.sort(() => Math.random() - Math.random())
    .slice(0, Number(process.env.REACT_APP_API_ELEMENTS_TO_PICK))
);

const ChooseMinifigs: React.FC = () => {
  const [minifigs, setMinifigs] = React.useState<MinifigInterface[] | null>(null);
  const [selectedMinifig, setSelectedMinifig] = React.useState<MinifigInterface | null>(null);
  const [isDrawerOpen, setIsDrawerOpen] = React.useState<boolean>(false);
  const [minifigsParts, setMinifigsParts] = React.useState<MinifigPartsResult[] | null>(null);

  const { data: minifigsData }: UseQueryResult<CommonResponse<MinifigInterface>, Error> = useQuery(
    'minifigs',
    fetchMinifigs,
    {
  ...
  useMemo(() => (
    minifigsData?.results && setMinifigs(filterRandomElements(minifigsData?.results))
  ), [minifigsData]);
